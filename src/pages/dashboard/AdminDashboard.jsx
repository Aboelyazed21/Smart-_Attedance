...
function AdminDashboard() {
  ...
  async function loadDashboard(isRefresh = false) {
    try {
      ...
      setError(err.message || t("adminDash.failed_to_load_dashboard_data"));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const stats = [
    {
      key: "students",
      label: t("adminDash.total_students"),
      value: dashboardData.students,
      description: t("adminDash.registered_students"),
      icon: "students",
      tone: "blue",
    },
    {
      key: "sessions",
      label: t("adminDash.total_sessions"),
      value: dashboardData.sessions,
      description: t("adminDash.attendance_sessions"),
      icon: "sessions",
      tone: "violet",
    },
    {
      key: "attendance",
      label: t("adminDash.attendance_records"),
      value: dashboardData.attendanceRecords,
      description: t("adminDash.accepted_attendance_2"),
      icon: "calendar",
      tone: "green",
    },
    {
      key: "corrections",
      label: t("adminDash.pending_corrections"),
      value: dashboardData.pendingCorrections,
      description: t("adminDash.awaiting_review"),
      icon: "pending",
      tone: "orange",
    },
  ];
      ...
      <main className="dashboard-main">
        <header className="dashboard-header">
          <div className="header-title-area">
            ...
            <p>
              {t("adminDash.welcome_back").replace("{name}", firstName)}
            </p>
          </div>
          ...
          <div className="dashboard-header-actions">
            <button
              ...
              disabled={refreshing}
              title={t("adminDash.refresh_dashboard")}
            >
              <Icon type="refresh" size={18} />
              <span>{refreshing ? t("adminDash.refreshing") : t("action.refresh")}</span>
            </button>
            ...
            <div className="header-account">
              ...
              <div className="header-account-info">
                ...
                <span>{t("adminDash.administrator")}</span>
              </div>
            </div>
          </div>
        </header>
        ...
        <section className="dashboard-content">
          <div className="dashboard-hero">
            <div>
              <span className="hero-eyebrow">{t("adminDash.system_overview")}</span>
              <h2>{t("adminDash.attendance_management_at_a_glance")}</h2>
              <p>
                {t("adminDash.monitor_students_sessions_attendance_act")}
              </p>
            </div>

            <div className="hero-badge">
              {t("adminDash.system_active")}
            </div>
          </div>
          ...
          {error && (
            <div className="dashboard-error">
              ...
              <div>
                <strong>{t("adminDash.unable_to_load_dashboard_data")}</strong>
                <p>{error}</p>
              </div>

              <button type="button" onClick={() => loadDashboard()}>
                {t("adminDash.try_again")}
              </button>
            </div>
          )}
          ...
          <div className="dashboard-grid">
            <section className="dashboard-panel overview-panel">
              <div className="panel-header">
                <div>
                  <span className="panel-kicker">{t("adminDash.statistics")}</span>
                  <h2>{t("adminDash.system_overview_2")}</h2>
                  <p>{t("adminDash.current_attendance_system_statistics")}</p>
                </div>
              </div>
              ...
              <div className="overview-list">
                <div className="overview-item">
                  <div className="overview-item-left">
                    ...
                    <div>
                      <strong>{t("adminDash.students")}</strong>
                      <small>{t("adminDash.registered_accounts")}</small>
                    </div>
                  </div>
                ...
                <div className="overview-item">
                  <div className="overview-item-left">
                    ...
                    <div>
                      <strong>{t("adminDash.sessions")}</strong>
                      <small>{t("adminDash.attendance_sessions")}</small>
                    </div>
                  </div>
                ...
                <div className="overview-item">
                  <div className="overview-item-left">
                    ...
                    <div>
                      <strong>{t("adminDash.accepted_attendance")}</strong>
                      <small>{t("adminDash.validated_attendance_records")}</small>
                    </div>
                  </div>
                ...
                <div className="overview-item">
                  <div className="overview-item-left">
                    ...
                    <div>
                      <strong>{t("adminDash.pending_corrections")}</strong>
                      <small>{t("adminDash.requests_awaiting_review")}</small>
                    </div>
                  </div>
            ...
            <section className="dashboard-panel quick-panel">
              <div className="panel-header">
                <div>
                  <span className="panel-kicker">{t("adminDash.shortcuts")}</span>
                  <h2>{t("adminDash.quick_actions")}</h2>
                  <p>{t("adminDash.frequently_used_administrator_actions")}</p>
                </div>
              </div>
              ...
              <div className="quick-actions">
                ...
                >
                  ...
                  <span className="quick-action-content">
                    <strong>{t("adminDash.manage_users")}</strong>
                    <small>{t("adminDash.view_and_manage_system_users")}</small>
                  </span>
                ...
                >
                  ...
                  <span className="quick-action-content">
                    <strong>{t("adminDash.manage_courses")}</strong>
                    <small>{t("adminDash.view_and_manage_courses")}</small>
                  </span>
                ...
                >
                  ...
                  <span className="quick-action-content">
                    <strong>{t("adminDash.view_reports")}</strong>
                    <small>{t("adminDash.analyze_attendance_reports")}</small>
                  </span>
          ...
          <section className="dashboard-panel activity-panel">
            <div className="panel-header">
              <div>
                <span className="panel-kicker">{t("adminDash.activity")}</span>
                <h2>{t("adminDash.system_status")}</h2>
                <p>{t("adminDash.quick_view_of_your_attendance_platform")}</p>
              </div>

              <span className="status-pill">
                <span />
                {t("adminDash.operational")}
              </span>
            </div>
            ...
            <div className="status-grid">
              <div className="status-card">
                ...
                <div>
                  <strong>{t("adminDash.students")}</strong>
                  <span>
                    {loading ? t("adminDash.loading") : t("adminDash.registeredCount").replace("{n}", dashboardData.students)}
                  </span>
                </div>
              </div>
              ...
              <div className="status-card">
                ...
                <div>
                  <strong>{t("adminDash.sessions")}</strong>
                  <span>
                    {loading ? t("adminDash.loading") : t("adminDash.createdCount").replace("{n}", dashboardData.sessions)}
                  </span>
                </div>
              </div>
              ...
              <div className="status-card">
                ...
                <div>
                  <strong>{t("adminDash.attendance")}</strong>
                  <span>
                    {loading
                      ? t("adminDash.loading_2")
              ...
              <div className="status-card">
                ...
                <div>
                  <strong>{t("adminDash.corrections")}</strong>
                  <span>
                    {loading
                      ? t("adminDash.loading_2")
